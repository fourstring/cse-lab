...
fuseserver_symlink(fuse_req_t req, const char *link, fuse_ino_t parent, const char *name) {
    ...
    if (found) {
        ...
        if (yfs->symlink(parent, link, name, ino) != yfs_client::OK) {
...
void fuseserver_readlink(fuse_req_t req, fuse_ino_t ino) {
    ...
    if (yfs->read(inum, st.st_size, 0, buf) != yfs_client::OK) {
        fuse_reply_err(req, EIO);
    } else {
...
#endif
    if (argc != 4) {
        fprintf(stderr, "Usage: yfs_client <mountpoint> <server_port> <lock_port>\n");
        exit(1);
    }
    mountpoint = argv[1];
