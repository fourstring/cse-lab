...
int main(int argc, char **argv) {
	...
	r = y1.get("b");
		CHECK(r == "200", "check 1 error");
	y1.set("b", "1200");
	r = y2.get("c");
		CHECK(r == "300", "check 2 error");
	y2.set("c", "2200");
	r = y3.get("a");
		CHECK(r == "100", "check 3 error");
	y3.set("a", "3200");

	y3.transaction_commit();
	CHECK_TRANS_EXCEPTION(y2.transaction_commit(), ydb_protocol::ABORT, "check 4 error");
	CHECK_TRANS_EXCEPTION(y1.transaction_commit(), ydb_protocol::ABORT, "check 5 error");
	...
	r = y2.get("b");
		CHECK(r == "200", "check 7 error");
	r = y2.get("c");
		CHECK(r == "3100", "check 8 error");
