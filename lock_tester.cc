...
test1(void) {
    ...
    lc[0]->acquire(a, LockKind::Exclusive);
    check_grant(a);
    lc[0]->release(a, LockKind::Exclusive);
    check_release(a);
    lc[0]->acquire(a, LockKind::Exclusive);
    check_grant(a);
    lc[0]->release(a, LockKind::Exclusive);
    ...
    lc[0]->acquire(a, LockKind::Exclusive);
    check_grant(a);
    lc[0]->acquire(b, LockKind::Exclusive);
    check_grant(b);
    lc[0]->release(b, LockKind::Exclusive);
    check_release(b);
    lc[0]->release(a, LockKind::Exclusive);
    check_release(a);
}
...
test2(void *x) {
    ...
    lc[i]->acquire(a, LockKind::Exclusive);
    printf("test2: client %d acquire done\n", i);
    check_grant(a);
    sleep(1);
    printf("test2: client %d release\n", i);
    check_release(a);
    lc[i]->release(a, LockKind::Exclusive);
...
test5(void *x) {
    ...
    for (int j = 0; j < 10; j++) {
        if (i < 5) lc[0]->acquire(a, LockKind::Exclusive);
        else lc[1]->acquire(a, LockKind::Exclusive);
        check_grant(a);
        printf("test5: client %d got lock\n", i);
        check_release(a);
        if (i < 5) lc[0]->release(a, LockKind::Exclusive);
        else lc[1]->release(a, LockKind::Exclusive);
    }
    return 0;
}
